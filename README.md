...
You must first pull the Oracle Server JRE 8 image from [Oracle Container Registry](https://container-registry.oracle.com).
...
```
> Notice: password cannot be too weak (e.g. mima1234, 123456 or password) that may result in OHS fails to boot.
...
1. To start the OHS container with oracle/ohs:12.2.1.3.0 image, run the following command:
         ...
         docker run --ulimit nofile=65536:65536 -v `HOST PATH where the domain.properties file is`:/u01/oracle/bootdir -it --name ohs -p 7777:7777 oracle/ohs:12.2.1.3.0
